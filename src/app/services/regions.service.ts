...
export class RegionsService {
  ...
  // full department url should be https://geo.api.gouv.fr/regions/53/departements
  private DEPARTMENTS_URL = 'https://geo.api.gouv.fr/regions';
  regionDepartments = signal<Department[]>([]);
  ...
  private departmentsEffect = effect(() => {
    ...
    this.http
      .get<Department[]>(
        `${this.DEPARTMENTS_URL}/${this.selectedRegionSig().Code}/departements`
      )
      .pipe(tap((data) => console.log({ departments: data })))
      .subscribe((departments) => this.regionDepartments.set(departments));
  });
